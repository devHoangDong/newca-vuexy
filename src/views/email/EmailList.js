import React, { useState, useEffect } from "react";
import axios from "axios";
import {
  FormGroup,
  Input,
  UncontrolledDropdown,
  DropdownToggle,
  DropdownItem,
  DropdownMenu,
  Media,
} from "reactstrap";
import {
  Search,
  Check,
  Folder,
  Edit2,
  Info,
  Trash,
  Tag,
  Mail,
  Star,
  Menu,
} from "react-feather";
import PerfectScrollbar from "react-perfect-scrollbar";
import { connect } from "react-redux";
import {
  getEmails,
  StarEmail,
  searchMail,
  moveMail,
  selectMail,
  selectAllMails,
  deselectAllMails,
  unreadMails,
  setLabel,
} from "../../redux/actions/email/index";
import Checkbox from "../../components/@vuexy/checkbox/CheckboxesVuexy";
import EmailDetails from "./EmailDetails";

const EmailList = (props) => {
  const [mails, setMails] = useState([]);
  const [emailDetailsVisibility, setEmailDetailsVisibility] = useState(false);
  const [selectAll, setSelectAll] = useState(false);
  const [currentEmail, setCurrentEmail] = useState([]);
  const [value, setValue] = useState("");
  const [currentFilter, setCurrentFilter] = useState(
    props.routerProps.match.params.filter
  );
  // getDerivedStateFromProps = (props, state) => {
  //   if (props.email.params !== state.currentFilter) {
  //     setMails(props.email.mails);
  //   } else {
  //     // Return null if the state hasn't changed
  //     return null;
  //   }
  // };

  const handleGetEmail = async () => {
    await axios.get("/api/email/mails").then(function (response) {
      setMails(response.data);
    });
  };

  const handleEmailDetails = (status, mail) => {
    if (status === "open") {
      setEmailDetailsVisibility(true);
      setCurrentEmail(mail);
    } else setEmailDetailsVisibility(false);
  };

  const handleNextMail = () => {
    if (mails.length) {
      let getIndex = mails.find((i) => i.id === currentEmail.id);
      let currentEmail = mails[mails.indexOf(getIndex) + 1];

      if (currentEmail !== undefined) {
        setCurrentEmail(currentEmail);
      }
    }
  };

  const handlePreviousMail = () => {
    if (mails.length) {
      let getIndex = mails.find((i) => i.id === currentEmail.id);
      let currentEmail = mails[mails.indexOf(getIndex) - 1];

      if (currentEmail !== undefined) {
        setCurrentEmail(currentEmail);
      }
    }
  };
  useEffect(() => {
    handleGetEmail();
  }, []);

  // const handleOnChange = (e) => {
  //   this.setState({ value: e.target.value });
  //   this.props.searchMail(e.target.value);
  // };
  const mailsArr = value.length ? props.email.filteredMails : mails;
  const renderMails =
    mailsArr.length > 0 ? (
      mailsArr.map((mail) => {
        return (
          <Media
            tag="li"
            key={mail.id}
            className={mail.unread === false ? "mail-read" : "mail-unread"}
            onClick={() => {
              handleEmailDetails("open", mail);
            }}
          >
            <Media className="pr-50" tag="div" left>
              <div className="avatar">
                <Media object src={mail.img} />
              </div>
              <div className="user-action">
                <Checkbox
                  color="primary"
                  className="user-checkbox"
                  icon={<Check className="vx-icon" size={12} />}
                  label={""}
                  checked={props.email.selectedEmails.includes(mail.id)}
                  size="sm"
                  onClick={(e) => {
                    props.selectMail(mail.id);
                    e.stopPropagation();
                  }}
                  onChange={(e) => e.stopPropagation()}
                />
                <div className="favorite">
                  <Star
                    size={18}
                    fill={mail.isStarred ? "#FF9F43" : "transparent"}
                    stroke={mail.isStarred ? "#FF9F43" : "#626262"}
                    onClick={(e) => {
                      props.StarEmail(mail.id);
                      e.stopPropagation();
                    }}
                  />
                </div>
              </div>
            </Media>
            <Media body>
              <div className="user-details flex-wrap">
                <div className="mail-items">
                  <h5 className="text-bold-600 mb-25 ">{mail.sender_name}</h5>
                  <span className="text-truncate">{mail.subject}</span>
                </div>
                <div className="mail-meta">
                  <span className="float-right">
                    {Array.isArray(mail.labels) ? (
                      <React.Fragment>
                        {mail.labels.map((label) => {
                          return (
                            <span
                              className={`bullet bullet-${
                                label === "important"
                                  ? "warning"
                                  : label === "personal"
                                  ? "success"
                                  : label === "private"
                                  ? "danger"
                                  : "primary"
                              } bullet-sm d-none d-md-inline-block mr-1`}
                              key={label}
                            />
                          );
                        })}
                      </React.Fragment>
                    ) : (
                      <span
                        className={`bullet ${mail.labels} bullet-sm mr-1`}
                      />
                    )}
                    <span className="mail-date">{mail.day}</span>
                  </span>
                </div>
              </div>
              <div className="mail-message">
                <p className="list-group-item-text truncate mb-0">
                  {mail.message}
                </p>
              </div>
            </Media>
          </Media>
        );
      })
    ) : (
      <div className="no-results show">
        <h5>No Items Found</h5>
      </div>
    );

  return (
    <div className="content-right">
      <div className="email-app-area">
        <div className="email-app-list-wrapper">
          <div className="email-app-list">
            <div className="app-fixed-search">
              <div
                className="d-lg-none sidebar-toggle"
                onClick={() => props.mainSidebar(true)}
              >
                <Menu size={24} />
              </div>
              <FormGroup className="position-relative has-icon-left m-0 d-inline-block d-lg-block">
                <Input
                  placeholder="Search Emails"
                  // onChange={(e) => {
                  //   this.handleOnChange(e);
                  // }}
                  value={value}
                />
                <div className="form-control-position">
                  <Search size={15} />
                </div>
              </FormGroup>
            </div>
            <div className="app-action">
              <div className="action-left">
                <Checkbox
                  color="primary"
                  icon={<Check className="vx-icon" size={16} />}
                  label="Select All"
                  checked={props.email.selectedEmails.length ? true : false}
                  onChange={(e) => e.stopPropagation()}
                  onClick={(e) => {
                    e.target.checked
                      ? props.selectAllMails()
                      : props.deselectAllMails();
                  }}
                />
              </div>
              <div className="action-right">
                <ul className="list-inline m-0">
                  <li className="list-inline-item">
                    <UncontrolledDropdown>
                      <DropdownToggle tag="div">
                        <Folder size={22} />
                      </DropdownToggle>
                      <DropdownMenu right>
                        <DropdownItem
                          onClick={() => {
                            if (currentFilter === "inbox")
                              props.moveMail("draft");
                            else props.moveMail("inbox");
                          }}
                        >
                          <Edit2 size={18} className="mr-50" />
                          <span className="align-middle font-medium-1">
                            {currentFilter === "inbox" ? "Draft" : "Inbox"}
                          </span>
                        </DropdownItem>
                        <DropdownItem
                          onClick={() => {
                            props.moveMail("spam");
                          }}
                        >
                          <Info size={18} className="mr-50" />
                          <span className="align-middle font-medium-1">
                            Spam
                          </span>
                        </DropdownItem>
                        <DropdownItem
                          onClick={() => {
                            props.moveMail("trash");
                          }}
                        >
                          <Trash size={18} className="mr-50" />
                          <span className="align-middle font-medium-1">
                            Trash
                          </span>
                        </DropdownItem>
                      </DropdownMenu>
                    </UncontrolledDropdown>
                  </li>
                  <li className="list-inline-item">
                    <UncontrolledDropdown>
                      <DropdownToggle tag="div">
                        <Tag size={22} />
                      </DropdownToggle>
                      <DropdownMenu right>
                        <DropdownItem
                          className="font-medium-1"
                          onClick={() => props.setLabel("personal")}
                        >
                          <span className="bullet bullet-success bullet-sm mr-1" />
                          <span className="align-middle">Personal</span>
                        </DropdownItem>
                        <DropdownItem
                          className="font-medium-1"
                          onClick={() => props.setLabel("company")}
                        >
                          <span className="bullet bullet-primary bullet-sm mr-1" />
                          <span className="align-middle">Company</span>
                        </DropdownItem>
                        <DropdownItem
                          className="font-medium-1"
                          onClick={() => props.setLabel("important")}
                        >
                          <span className="bullet bullet-warning bullet-sm mr-1" />
                          <span className="align-middle">Important</span>
                        </DropdownItem>
                        <DropdownItem
                          className="font-medium-1"
                          onClick={() => props.setLabel("private")}
                        >
                          <span className="bullet bullet-danger bullet-sm mr-1" />
                          <span className="align-middle">Private</span>
                        </DropdownItem>
                      </DropdownMenu>
                    </UncontrolledDropdown>
                  </li>
                  <li
                    className="list-inline-item mail-unread"
                    onClick={() => props.unreadMails(true)}
                  >
                    <span className="action-icon">
                      <Mail size={22} />
                    </span>
                  </li>
                  <li
                    className="list-inline-item mail-delete"
                    onClick={() => {
                      props.moveMail("trash");
                    }}
                  >
                    <span className="action-icon">
                      <Trash size={22} />
                    </span>
                  </li>
                </ul>
              </div>
            </div>
            <PerfectScrollbar
              className="email-user-list list-group"
              options={{
                wheelPropagation: false,
              }}
            >
              <ul className="users-list-wrapper media-list">{renderMails}</ul>
            </PerfectScrollbar>
          </div>
        </div>
        <EmailDetails
          handleEmailDetails={handleEmailDetails}
          currentStatus={emailDetailsVisibility}
          currentEmail={currentEmail}
          toggleStarred={props.StarEmail}
          setLabel={props.setLabel}
          unreadMails={props.unreadMails}
          currentParam={props.routerProps.match.params}
          moveMail={props.moveMail}
          handleNextMail={handleNextMail}
          handlePreviousMail={handlePreviousMail}
        />
      </div>
    </div>
  );
};
const mapStateToProps = (state) => {
  return {
    email: state.emailApp.mails,
    starred: state.emailApp.starred,
  };
};
export default connect(mapStateToProps, {
  getEmails,
  StarEmail,
  searchMail,
  moveMail,
  selectMail,
  selectAllMails,
  deselectAllMails,
  unreadMails,
  setLabel,
})(EmailList);
